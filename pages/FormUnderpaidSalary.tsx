...
import { Complaint, ComplaintType, UnderpaidSalaryComplaint, ComplaintStatus, Worker } from '../types';
...
const FormUnderpaidSalary: React.FC<Props> = ({ workers, previousComplaints, onSubmit, onBack }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const complaint: UnderpaidSalaryComplaint = {
      ...formData,
      id: Math.random().toString(36).substr(2, 9),
      type: ComplaintType.UNDERPAID_SALARY,
      status: ComplaintStatus.PENDING,
      timestamp: new Date().toISOString(),
      evidenceUrl: 'https://picsum.photos/400/300' // Mock image URL
    };
    onSubmit(complaint);
  };
    ...
    <div className="max-w-xl mx-auto bg-white p-8 rounded-2xl shadow-sm border animate-fadeIn">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-blue-600 flex items-center">
          <span className="mr-2">💰</span> Form Komplen Kurang Gaji
        </h2>
        <p className="text-slate-500 text-xs mt-1">Sistem otomatis mengenali data Anda untuk mempercepat proses pengisian.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="relative" ref={suggestionRef}>
            ...
            <div className="relative">
              <input 
                ...
                className={`w-full px-4 py-3 rounded-lg border transition-all outline-none ${
                  ...
                  isVerified === false ? 'border-red-500 bg-red-50' : 'border-slate-200 focus:ring-2 focus:ring-blue-500'
                }`}
              />
                ...
                {suggestions.map((s, i) => (
                  ...
                  >
                    <div className="flex flex-col">
                      <span className="font-black text-xs text-blue-600">{s.opsId}</span>
                      <span className="text-sm font-bold text-slate-700">{s.fullName}</span>
                    </div>
                    <svg className="w-4 h-4 text-slate-300 group-hover:text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"></path></svg>
                  </button>
                ))}
              </div>
            )}
            ...
            <p className={`text-[10px] mt-1 font-bold uppercase ${isVerified === true ? 'text-green-600' : isVerified === false ? 'text-red-500' : 'text-slate-400'}`}>
              {isVerified === true ? (isAutofilled ? 'Profil & Rekening Ditemukan' : 'Karyawan Terdaftar') : isVerified === false ? 'ID Tidak Terdaftar' : 'Wajib OpsID'}
            </p>
          </div>
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1">Nomor WhatsApp (Aktif)</label>
            <input required type="tel" value={formData.whatsappNumber} onChange={e => setFormData({...formData, whatsappNumber: e.target.value})} placeholder="0812..." className={`w-full px-4 py-3 rounded-lg border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none transition-all ${isAutofilled ? 'bg-blue-50/50' : ''}`} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select required value={formData.bankName} onChange={e => setFormData({...formData, bankName: e.target.value})} className={`w-full px-4 py-3 rounded-lg border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none bg-white transition-all ${isAutofilled ? 'bg-blue-50/50' : ''}`}>
          ...
          <div>
            ...
            <input required type="number" value={formData.bankAccountNumber} onChange={e => setFormData({...formData, bankAccountNumber: e.target.value})} placeholder="No Rekening" className={`w-full px-4 py-3 rounded-lg border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none transition-all ${isAutofilled ? 'bg-blue-50/50' : ''}`} />
          </div>
        </div>

        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1">Nama Penerima di Rekening</label>
          <input required type="text" value={formData.bankAccountName} onChange={e => setFormData({...formData, bankAccountName: e.target.value})} placeholder="Nama Pemilik Rekening" className={`w-full px-4 py-3 rounded-lg border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none transition-all ${isAutofilled ? 'bg-blue-50/50' : ''}`} />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1">Periode Masuk</label>
            <select 
              required 
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1">Gaji yang Diterima (Rp)</label>
            <input required type="number" value={formData.receivedAmount} onChange={e => setFormData({...formData, receivedAmount: e.target.value})} placeholder="Masukkan Nominal" className="w-full px-4 py-3 rounded-lg border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none" />
          </div>
        </div>

        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-2">Upload Bukti Mutasi Rekening</label>
          <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-200 border-dashed rounded-xl hover:border-blue-400 transition-colors cursor-pointer bg-slate-50">
        ...
        <div className="pt-4 flex flex-col space-y-3">
          <button 
            ...
            className={`w-full py-4 rounded-xl font-bold transition-all shadow-lg ${
              isVerified ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-blue-200 active:scale-[0.98]' : 'bg-slate-200 text-slate-400 cursor-not-allowed shadow-none'
            }`}
          >
            Kirim Komplen
          </button>
...
export default FormUnderpaidSalary;
